...
let newSoot = new Soot (400, 750, 40, 50, '/img/sootball.png', 'image', 0);
...
function gameLoop () {
  if (gameLoopTrigger === true) {
    ...
    if (collisionDetectionWater() && !collisionDetectionHarmless()) {
      updateDom();
    }
    if (collisionDetectionFirePit() && !reachFire()) {
      updateDom();
    }
    if (collisionDetectionDeadly() || reachFire()) {
      displayScore();
      restartRound();
    }
    resetLives();

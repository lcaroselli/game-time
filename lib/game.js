...
let score = 0;
let highScore = 0; //NEED HIGHSCORE TO CAPTURE INTEGER IN LOCAL STORAGE
  ...
  for (var i = 0; i < deadlyToGenerate.length; i++) {
   ...
   deadlyToGenerate[i].height + deadlyToGenerate[i].y > newSoot.y) {
     sootLives--
      ...
      return true;
    // If lost all 3 lives, then game over page and reset score value
    }
  }
}
...
// function storeHighScore() {
  ...
  //if score is more than current high score, then update high score to score's value and reset in JSON
